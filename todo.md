...
- [ ] **Chunk 2 — the map player without the tour (spec sections 2 to 9, 10, 14).** The biggest chunk; split the panels and lightbox (section 6) off as "2b" if it gets too large.
  - Suggested order: (1) DOM-free `src/mapModel.js` plus `scripts/check_map.mjs` (located events, scopes, shared-dot grouping, numbering, segments, bearing, bounds, stepping, settings), so the logic is pinned first; (2) vendor Leaflet **1.9.4** under `vendor/leaflet/` (`leaflet.js`, `leaflet.css`, licence; not the 2.0 alpha) and the tile source list; (3) launcher panel and full-viewport stage in `src/mapPlayer.js`, dots, lines, arrows, numbers; (4) left and right panels, pinning, lightbox, stepping, keys, settings memory; (5) export inlining of Leaflet and its CSS **only when Map is the chosen player**, escaping so it can't close its `<script>`, no leftover `url(` in the inlined CSS, and `check_export.mjs` assertions; (6) the player-contract edits in spec section 10, register in `players.js` / `playerRenderers.js`, flip `available`, add the CSS to `player.html` and `PLAYER_CSS_URLS`, add the modules to `check_js.py` (vendor files excluded from the syntax check).
  - Gotchas: `exportBundle.js` requires every top-level name to be unique across bundled modules, and only understands plain `import { a } from "./x.js"` and `export function|const|let|class`. Leaflet is a classic script and must not go through the bundler. Do not call the geocoder from the player.
  - Must be tried by hand before it counts as done: a real `.timeline.html` **opened from disk** (tiles, and the Referer question in spec section 4), tiles offline, panels and pinning on resize, small screens.
  - Update the spec's "not built" wording and the README ("nothing reads it yet") when this lands.
- [ ] **Chunk 3 — pin on map and "Look up missing coordinates" (spec 1.3 and 1.4).** Reuses chunk 2's Leaflet setup and tile list, so it comes after. The pin dialog sets coordinates through `coordinatesField.setGeo({ ..., source: "map" })`. Restore the "or pick on the map" wording in the "couldn't find" message in `coordinatesField.js` when the button exists. The batch action needs the same consent as the editor lookup and goes through the same geocoder queue.
- [ ] **Later — auto-play tour (spec section 11).** Not part of the first version. The spec flags a risk: Leaflet's `flyTo` can drift from a separately moved marker. Prototype driving both from one animation-frame loop before promising a smooth result.
