...
- [x] **Chunk 2 — the map player without the tour (spec sections 2 to 9, 10, 14). Built; still needs a hand check on a real device.**
  ...
  - **Left out:** the save dialog does not yet name the tile hosts when Map is chosen (spec section 10, item 2; contract 2.6 lists it as a gap). `renderPlaceholderPlayer` in `playerRenderers.js` is now unused.
